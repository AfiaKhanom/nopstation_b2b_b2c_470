    ...
    public class SqlReportController : NopStationAdminController
    {
        ...
        protected async Task<IList<SqlReportFilterOptionModel>> ParseFormAsync(IFormCollection form)
        {
            ...
            foreach (var key in form.Keys)
            {
                ...
                var sqlReportFilterOptionModel = new SqlReportFilterOptionModel()
                {
                    SystemName = tokens[1],
                ...
                switch (parameter.DataType)
                {
                    ...
                    case DataType.InputText:
                        sqlReportFilterOptionModel.IsTextInputItem = true;
                        sqlReportFilterOptionModel.FormattedValue = val.FirstOrDefault();
                        break;
                    case DataType.Number:
                        if (decimal.TryParse(val.FirstOrDefault(), out var numVal))
                            sqlReportFilterOptionModel.FormattedValue = Convert.ToString(numVal);
                        else
                            continue;
                        break;
                    case DataType.TextList:
                        sqlReportFilterOptionModel.IsListItem = true;
                        var textListVal = val.Any() ? val.ToList() : new List<string>();
                        sqlReportFilterOptionModel.FormattedValue = string.Join(",", textListVal.Select(x => "'" + x + "'"));
                        break;
                    case DataType.NumberList:
                        sqlReportFilterOptionModel.IsListItem = true;
                        var numListVal = val.Any() ? val.Select(int.Parse).ToList() : new List<int>();
                        sqlReportFilterOptionModel.FormattedValue = string.Join(",", numListVal);
                        break;
                    case DataType.Date:
                        if (DateTime.TryParse(val.FirstOrDefault(), out var dateVal))
                        {
                            sqlReportFilterOptionModel.IsDateItem = true;
                            sqlReportFilterOptionModel.FormattedValue = $"'{dateVal.ToString("yyyy-MM-ddTHH:mm:ss")}'";
                        }
                        break;
                ...
                list.Add(sqlReportFilterOptionModel);
            }
        ...
        public async Task<IActionResult> ExportReport(IFormCollection form)
        {
            ...
            if (result == null)
            {
                _notificationService.ErrorNotification("No data found");
                return RedirectToAction("ViewReport", new { id = id });
            }
            //return File(result, "text/xls", $"{sqlReport.Name ?? "Report"}.xlsx");
            return File(result, MimeTypes.TextXlsx, $"{sqlReport.Name ?? "Report"}.xlsx");
        }
