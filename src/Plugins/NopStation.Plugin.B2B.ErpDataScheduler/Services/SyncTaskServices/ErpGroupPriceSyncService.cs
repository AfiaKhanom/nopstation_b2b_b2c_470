            ...
            try
            {
                ...
                foreach (var salesOrg in allErpSalesOrgs)
                {
                    ...
                    while (true)
                    {
                        ...
                        {
                            var product = await _productService.GetProductBySkuAsync(erpGroupPrice.ItemNo);
                            ...
                            if (!string.IsNullOrEmpty(erpGroupPrice.PriceGroupCode))
                            {
                                var oldErpGroupPriceCode = await _erpGroupPriceCodeService.GetErpGroupPriceCodeByCodedAsync(erpGroupPrice.PriceGroupCode) ?? new ErpGroupPriceCode();
                                ...
                                {
                                    oldErpGroupPriceCode.Code = erpGroupPrice.PriceGroupCode ?? string.Empty;
                                    oldErpGroupPriceCode.LastUpdateTime = DateTime.UtcNow;
                                ...
                                {
                                    oldErpGroupPrice.Price = erpGroupPrice.SellingPrice ?? 0;
                            ...
                            if (erpGroupPrice.Prices.Any())
                            {
                                foreach (var price in erpGroupPrice.Prices)
                                {
                                    if (price.Value > 0)
                                    {
