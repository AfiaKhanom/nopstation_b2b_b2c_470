...
namespace NopStation.Plugin.B2B.ERPIntegrationCore.Services
{
    public class ErpSalesRepService : IErpSalesRepService
    {
        ...
        public virtual async Task DeleteErpSalesRepsAsync(IList<ErpSalesRep> erpSalesReps)
        {
        ...
        /// <summary>
        /// Gets an ErpSalesRep by Id
        /// </summary>
        /// <param name="id">ErpSalesRep identifier</param>
        /// <returns>
        /// A task that represents the asynchronous operation
        /// The task result contains the ErpSalesRep
        /// </returns>
        ...
        public virtual async Task<IList<ErpSalesRep>> GetErpSalesRepByIdsAsync(int[] erpSalesRepIds)
        {
            return await _erpSalesRepRepository.GetByIdsAsync(erpSalesRepIds, includeDeleted: false);
        }

        /// <summary>
        /// Gets an ErpSalesRep by Id if it is active
        /// </summary>
        /// <param name="id">ErpSalesRep identifier</param>
        /// <returns>
        /// A task that represents the asynchronous operation
        /// The task result contains the ErpSalesRep if it is activ
        /// </returns>
        ...
        /// <returns>
        /// A task that represents the asynchronous operation
        /// The task result contains all the ErpSalesRep
        /// </returns>
        public async Task<IPagedList<ErpSalesRep>> GetAllErpSalesRepAsync(int nopCustomerId = 0, int salesRepTypeId = 0,
            int pageIndex = 0, int pageSize = int.MaxValue, bool showHidden = false, bool getOnlyTotalCount = false,
        ...
        public async Task<IPagedList<ErpNopUser>> GetAllSalesRepUsersAsync(int salesRepId = 0, string erpAccontNo = null,
            string accountName = null, string email = null, string fullName = null,
            ...
            bool getOnlyTotalCount = false)
        {
            var erpNopUsers = await _erpNopUserRepository.GetAllPagedAsync(async query =>
            {
                ...
                var erpSalesRepId = (await _customerServive.GetCustomerRoleBySystemNameAsync(ERPIntegrationCoreDefaults.B2BSalesRepRoleSystemName)).Id;
                ...
                //filter by customer roles(must have b2b/b2c role)
                var nopUserAndNopUserAccountMapJoin = (from n in query
                                                       join m in _erpNopUserAccountMapRepository.Table
                                                       on n.Id equals m.ErpUserId
                                                       select new { nopUser = n, mapping = m }).ToList();

                var selectedNopUsers = from obj in nopUserAndNopUserAccountMapJoin
                                       let ids = obj.mapping.CustomerRolesIds.Split(',').Select(p => Convert.ToInt32(p.Trim())).ToList()
                                       where !ids.Contains(erpSalesRepId) && (ids.Contains(b2CCustomerRoleId) || ids.Contains(b2BCustomerRoleId))
                                       select obj.nopUser;
        ...
        public async Task<IPagedList<ErpNopUser>> GetAllSalesRepUsersBySalesRepIdAsync(int salesRepId = 0, string erpAccontNo = null,
            string accountName = null, string email = null, string fullName = null,
            int pageIndex = 0, int pageSize = int.MaxValue, bool showHidden = false,
            bool getOnlyTotalCount = false)
        {
        ...
        public virtual async Task<IList<ErpSalesOrg>> GetSalesRepOrgsAsync(int salesRepId, bool showHidden = false)
        {
        ...
        public virtual async Task<IPagedList<Customer>> GetAllSalesRepCustomersAsync(int pageIndex = 0, int pageSize = int.MaxValue, bool getOnlyTotalCount = false)
        {
        ...
        public virtual async Task<IPagedList<Customer>> GetAllCustomersNotYetSalesRepAsync(int includeSalesRepCustomerId = 0, int pageIndex = 0, int pageSize = int.MaxValue, bool getOnlyTotalCount = false)
        {
            var registeredCustomerRole = await _customerServive.GetCustomerRoleBySystemNameAsync(NopCustomerDefaults.RegisteredRoleName);
            var customers = await _customerRepository.GetAllPagedAsync(query =>
            {
        ...
        #region Utilities
        private bool HasCustomerRole(string customerRolesIds, int roleId)
        {
            if (customerRolesIds == null)
                return false;
            var x = customerRolesIds.Split(',').Select(x => Convert.ToInt32(x.Trim())).ToList();
            return x.Contains(roleId);
        }
