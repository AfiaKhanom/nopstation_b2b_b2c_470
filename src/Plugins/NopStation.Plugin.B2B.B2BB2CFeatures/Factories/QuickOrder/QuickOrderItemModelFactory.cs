...
namespace NopStation.Plugin.B2B.B2BB2CFeatures.Factories.QuickOrder
{
    public class QuickOrderItemModelFactory : IQuickOrderItemModelFactory
    {
        ...
        public QuickOrderItemModelFactory(
        ...
        private async Task<(PropertyManager<Product, Nop.Core.Domain.Localization.Language> Manager, IList<PropertyByName<Product, Language>> Properties, PropertyManager<ImportProductMetadata, Language> ProductAttributeManager)> PrepareImportDataAsync(IXLWorkbook workbook, IXLWorksheet worksheet)
        {
            ...
            var model = await new QuickOrderItemListModel().PrepareToGridAsync(searchModel, quickOrderItems, () =>
            {
                return quickOrderItems.SelectAwait(async quickOrder =>
                {
                    ...
                    if (product != null)
                    {
                        ...
                        quickOrderItem.UOM = await _erpSpecificationAttributeService.GetProductUOMByProductIdAndSpecificationAttributeId(product.Id, b2Bb2CCustomerAccountSettings.PreFilterFacetSpecificationAttributeId) ?? string.Empty;
        ...
        public async Task<IList<string>> ImportQuickOrderItemsFromXlsxAsync(int templateId, Stream stream)
        {
            ...
            try
            {
                ...
                var defaultWorksheet = metadata.Manager.GetDefaultProperties;
